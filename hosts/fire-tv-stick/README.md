...
**Automated since (pending install, see below):** the manual
"push a new .lpl after downloading more titles" step above is superseded
by `firetv-playlist-sync.timer`, a 5-minute poll on `overmind-01` (script:
`scripts/playlist_sync.py`). R-Shop is a sideloaded APK with no local
database and no add/remove hooks (confirmed by reverse-engineering, see
below) -- it reads its SMB source live and just writes downloaded ROMs
into `/storage/DF3B-5BC7/roms/<platform>/`, so that directory's own
contents are the only reliable change signal available. Each run: `adb
shell ls` the real device directory, diff against last-known state at
`/var/lib/overmind/fire-tv-roms/<platform>.json`, and if it changed,
rebuild the `.lpl` from the actual file listing (not the curated library
tier) and `adb push` it over the old one. Deliberately conservative: skips
the cycle entirely (no push) if the device is unreachable, or if a
listing comes back empty while the prior state wasn't -- that's treated
as a suspect transient read, not "the library was emptied."

This needs its own always-on `adb` session to the Fire TV Stick, which
doesn't exist yet -- prior `adb` use here has all been manual, from a Mac,
during provisioning. Mirrors `overmind-01`'s existing
`pixel-adb-reconnect.timer` pattern (see
[hosts/overmind-01/README.md](../overmind-01/README.md)) for a different
device:

```ini
# /etc/systemd/system/firetv-adb-reconnect.service
[Unit]
Description=Reconnect adb to the Fire TV Stick (self-heal for dropped adb tcpip sessions)
...
Type=oneshot
User=austin
ExecStart=/usr/bin/adb connect 192.168.68.62:5555
```

```ini
# /etc/systemd/system/firetv-adb-reconnect.timer
[Unit]
Description=Periodically reconnect adb to the Fire TV Stick
...
```ini
# /etc/systemd/system/firetv-playlist-sync.service
[Unit]
Description=Sync RetroArch .lpl playlists from the Fire TV Stick's real on-device ROM directories
After=firetv-adb-reconnect.service

[Service]
Type=oneshot
User=austin
ExecStart=/usr/bin/python3 /opt/overmind/hosts/fire-tv-stick/scripts/playlist_sync.py
```

```ini
# /etc/systemd/system/firetv-playlist-sync.timer
[Unit]
Description=Periodically sync RetroArch playlists from the Fire TV Stick
...
Not committed as live files, installed manually on `overmind-01` (same
convention as `pixel-adb-reconnect`). Two prerequisites, neither doable
remotely, before these can actually be installed:

1. **DHCP reservation for the Fire TV Stick.** Its LAN IP
   (`192.168.68.62`) is currently unreserved DHCP -- needs a router-side
   reservation, same as `overmind-01` already has, or the timer silently
   starts talking to nothing after the next lease renewal.
2. **On-device ADB authorization for `overmind-01` as a new debugging
   client.** ADB debug keys are per-client; the Mac's authorized key
   doesn't carry over. The first `adb connect` from `overmind-01` pops an
   "Allow wireless debugging?" prompt on the TV screen itself, needing a
   physical remote click (pick "always allow this computer" so it
   survives reboots).

Once both are done: `systemctl daemon-reload && systemctl enable --now
firetv-adb-reconnect.timer firetv-playlist-sync.timer` on `overmind-01`.
Verify with `adb -s 192.168.68.62:5555 devices` (should show
`192.168.68.62:5555 device`, not `unauthorized`/`offline`) and
`journalctl -u firetv-playlist-sync.service` after the next download.
Rollback: `systemctl disable --now` both timers, delete the four unit
files -- no state is touched by removing them, and the last-pushed `.lpl`
files are left in place on the device as plain files.
